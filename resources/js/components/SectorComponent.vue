<template>
    <div class="app-container">
        <div class="container">
            ...
            <!-- Search Form -->
            <form @submit.prevent="fetchSectors" class="search-form">
            ...
            <div class="sector-card">
                <div v-for="(sector, index) in Sector" :key="sector.id" class="card">
                    <img :src="sector.image" alt="Sector Image" class="card-image" />
                    <div class="card-content">
                        <h3>{{ sector.title }}</h3>
                        <p>{{ sector.text }}</p>
                    </div>
                    <div class="card-buttons">
                        <button @click="openEditModal(sector)" class="edit-button">
                            <i class="fa fa-edit"></i>
                        </button>
                        <button @click="openDeleteModal(sector.id)" class="delete-button">
        ...
        <div v-if="showModal" class="modal-overlay">
            <div class="modal">
                ...
                <form @submit.prevent="submitForm">
                    ...
                    <textarea rows="4" class="input" autocomplete="off" v-model="form.text" placeholder="Content..." required />
...
const GetAlLSectors = async () => {
    try {
        const response = await axios.get('/api/get-sectors', {
            params: { search: searchQuery.value, page: page.value },
        });
        Sector.value = response.data.data.data;
...
const openAddModal = () => {
    modalTitle.value = 'Create New Sector';
...
const submitForm = async () => {
    try {
        ...
        if (form.value.id) {
            ...
            await axios.post(`/sector/${form.value.id}`, formData);
        } else {
            // console.log(formData);
            await axios.post('/sector', formData);
        }
...
const confirmDelete = async () => {
    try {
        await axios.delete(`/sector/${sectorIdToDelete.value}`);
        closeDeleteModal();
        await GetAlLSectors();
    } catch (error) {
