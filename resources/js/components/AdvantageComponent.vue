<template>
    <div class="app-container">
        <div class="container">
            ...
            <!-- Search Form -->
            <form @submit.prevent="fetchAdvantages" class="search-form">
            ...
            <div class="sector-card">
                <div v-for="(advantage, index) in Advantage" :key="advantage.id" class="card">
                    <div class="advantage-image">
                        <img :src="advantage.image" alt="Advantage Image" class="card-image" />
                    </div>
                    <div class="card-content">
                        <p>{{ advantage.text }}</p>
                    </div>
                    <div class="card-buttons">
                        <button @click="openEditModal(advantage)" class="edit-button">
                            <i class="fa fa-edit"></i>
                        </button>
                        <button @click="openDeleteModal(advantage.id)" class="delete-button">
        ...
        <div v-if="showModal" class="modal-overlay">
            <div class="modal">
                ...
                <form @submit.prevent="submitForm">
                    <textarea rows="2" class="input" autocomplete="off" v-model="form.text" placeholder="Content..." required />
...
const form = ref({ id: null,  text: '',image:null});
const advantageIdToDelete = ref(null);

const GetAlLAdvantages = async () => {
    try {
        const response = await axios.get('/api/get-advantages', {
            params: { search: searchQuery.value, page: page.value },
        });
        Advantage.value = response.data.data.data;
...
const openAddModal = () => {
    modalTitle.value = 'Create New Advantage';
    form.value = { id: null, text: '' ,image:null};
    showModal.value = true;
};

const openEditModal = (sector) => {
    modalTitle.value = 'Edit Advantage';
    form.value = { ...sector };
    showModal.value = true;
};
...
const submitForm = async () => {
    try {
        ...
        if (form.value.id) {
            ...
            await axios.post(`/advantage/${form.value.id}`, formData);
        } else {
            // console.log(formData);
            await axios.post('/advantage', formData);
        }
...
const confirmDelete = async () => {
    try {
        await axios.delete(`/advantage/${advantageIdToDelete.value}`);
        closeDeleteModal();
        await GetAlLAdvantages();
    } catch (error) {
